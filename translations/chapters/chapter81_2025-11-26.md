...
| 利 (lì) | 禾 + 刂 | Cut paths, benefit, advantage |
...
利 (lì) = the path-cutting constraint from the framework. Boundaries that shape possibility. Heaven's pattern cuts paths—creates constraints that enable function—without harming.

This is the 利 + 無 → 用 formula: constraint shapes void into function. Heaven does this naturally—provides the structuring without damaging what's structured.
